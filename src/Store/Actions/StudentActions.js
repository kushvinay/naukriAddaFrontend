...
//forget password
export const asyncforgetpasswordstd = (email) => async (dispatch, getState) =>{
   
  try {
    const data = await axios.post("/student/forget-password", email)
    console.log(data)
    dispatch(asyncCurrentUser())
 ...
 export const asyncotppasswordstudent = (pwd) => async (dispatch, getState) => {
  try {
    const otp  = await axios.post("/student/forget-link/", pwd);
    dispatch(asyncCurrentUser());
  } catch (error) {
    console.log(error)
    // dispatch(iserror(error.response.data.message));
